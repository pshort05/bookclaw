...
- Cloud/zip share of a whole world. Phase-12 transfer exists for other library kinds, but a documents-aware world `.zip` is a follow-up — so **Phase 1 explicitly rejects `world` in `LibraryTransferService` export/import** (`'world transfer is not supported yet (Phase 1)'`) rather than letting it fall through the directory-of-`.md` path. A later phase can implement a real world `.zip` (config + documents) and remove the guard.
