...
**Deploying (full runbook: [`docs/internal/DEPLOYMENT.local.md`](docs/internal/DEPLOYMENT.local.md), gitignored/setup-specific).** `git push` is version control only — **not** a deploy trigger; both hosts build **from the on-disk tree** at `/home/paul/data/dev/bookclaw` (shared inode across the workstation + both hosts), including uncommitted edits.
- **Mercury** (dev/test, `http://192.168.1.32:3847`, container `bookclaw`): `touch build_now` in the repo root. A systemd timer on Mercury polls the sentinel ~every 60s and rebuilds the on-disk tree via `scripts/build-watch.sh` (logs in `.build-logs/`), then consumes the sentinel. No git pull. Real-money smokes run from any LAN box against that URL (see [Testing](#testing)).
- **Neptune** (production, live book data, `http://192.168.1.28:3847`, container `bookclaw-writing`): manual and backup-first. `ssh paul@192.168.1.28`, back up workspace/db/vault/env, then `docker compose -p bookclaw-writing --env-file /mnt/bckup1/bookclaw-writing/.env.writing -f docker/docker-compose.yml -f docker/docker-compose.writing.yml up -d --build`. Exact commands + verification in `docs/internal/DEPLOYMENT.local.md`.

## Feature tracking workflow

All features for this project are tracked in two files:

- **`docs/internal/TODO.md`** — every pending feature, cleanup, investigation, and larger item. Anything currently being worked on must appear here.
- **`docs/internal/COMPLETED.md`** — items moved out of `TODO.md` once finished, with a completion date.

Rules for Claude:

- **Every feature the user is working on must be in `docs/internal/TODO.md`.** When the user describes new work that is not already listed, **prompt them to add it to the TODO before starting** — do not silently begin work on an untracked item.
...
- **Security perimeter env vars** (from the security review — see `docs/internal/COMPLETED.md`). Each is opt-out or opt-in via env; the constructor wires them and startup logs the posture:
...
- **Workspace storage is a host bind-mount in Docker** (`BOOKCLAW_WORKSPACE_PATH`, default `/home/paul/bookclaw-workspace`; changed from a named volume 2026-06-05 — Phase 0 of [docs/developer/BOOK-CONTAINER-ARCHITECTURE.md](docs/developer/BOOK-CONTAINER-ARCHITECTURE.md)). The container runs as the baked `bookclaw` user (uid 999) and Docker does **not** chown a host bind-mount, so `deploy.sh` aligns ownership after the build (`docker compose run --user 0 --entrypoint chown bookclaw -R bookclaw:bookclaw /app/workspace`) — a freshly created host dir would otherwise crash the app on its first write under `/app/workspace`. The `bookclaw-vault` volume stays a separate named volume.
