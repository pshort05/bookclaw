...
- `workspace/books/<slug>/` — per-book container created by `BookService` (book-container Phase 2): `book.json` manifest (`schemaVersion`-gated: too-old→quarantine, too-new→read-only) + `templates/` snapshot (author/genre/pipeline/sections copied from the library at create time) + `data/` outputs. Not yet driving generation (Phase 3 wires `SoulService`/`ProjectEngine` to read the active book). Genre is now injected into generation prompts via `BookService.getActiveGenreGuide()` → `buildSystemPrompt` (book-container Phase 7).
