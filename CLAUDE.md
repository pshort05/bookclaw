...
**Deploying (full runbook: [`docs/DEPLOYMENT.local.md`](docs/DEPLOYMENT.local.md), gitignored/setup-specific).** `git push` is version control only — **not** a deploy trigger; both hosts build **from the on-disk tree** at `/home/paul/data/dev/bookclaw` (shared inode across the workstation + both hosts), including uncommitted edits.
...
- **Neptune** (production, live book data, `http://192.168.1.28:3847`, container `bookclaw-writing`): manual and backup-first. `ssh paul@192.168.1.28`, back up workspace/db/vault/env, then `docker compose -p bookclaw-writing --env-file /mnt/bckup1/bookclaw-writing/.env.writing -f docker/docker-compose.yml -f docker/docker-compose.writing.yml up -d --build`. Exact commands + verification in `docs/DEPLOYMENT.local.md`.
