...
> **Notice — active development; wait for the official v6 release.** BookClaw is undergoing rapid, frequent changes right now. **Do not start a new novel on the current setup** — wait for the official **v6 release**, which will be the first stable baseline for new projects. Until then, expect breaking changes between commits (dashboard, pipelines, prompts/skills storage, config, and deploy scripts), and treat any in-progress work as disposable.
