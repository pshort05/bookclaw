 ...
 */
export function ModelPicker({ value, onChange, disabled, hideTemperature }: { value: ModelValue; onChange: (v: ModelValue) => void; disabled?: boolean; hideTemperature?: boolean }) {
  ...
  return (
    <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      {/* Catalog providers (openrouter/claude/gemini) get a real dropdown of models;
          others keep free-text. OpenRouter also offers "Newest Sonnet (auto)", the
          sentinel that resolves to the latest Sonnet at run time. A pinned custom
          slug not in the catalog is preserved as its own option. */}
      {provider !== '' && (
        CATALOG_PROVIDERS.has(provider) ? (
          <select value={value.model ?? ''} disabled={disabled} onChange={(e) => emit({ model: e.target.value })}>
            ...
            {value.model && value.model !== 'auto:newest-sonnet' && !models.some((m) => m.id === value.model) && (
