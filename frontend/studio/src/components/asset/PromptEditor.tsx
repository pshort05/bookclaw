...
/**
 * Prompt editor — a reusable writing-craft prompt (a JSON `prompt` library
 * kind) you run against a book file from the Prompt Runner. Mirrors
 * EditorEditor's load/save/scope conventions; serializes to { schemaVersion,
 * name, label, description, systemPrompt, model, temperature } via the same
 * library write path.
 */
export function PromptEditor({ scope, kind, name, displayName }: Props) {
  ...
  useEffect(() => {
    ...
    readEntry(scope, kind, name)
      .then((entry) => {
        let pd: PromptData | null = null;
        if (entry.prompt) {
          pd = entry.prompt;
        } else if (typeof entry.content === 'string' && entry.content.trim()) {
          try { pd = JSON.parse(entry.content); } catch { /* handled below */ }
        }
        if (!pd) {
          setError('Could not load this prompt (invalid or empty JSON).');
          return;
        }
        setLabel(pd.label ?? '');
        setDescription(entry.description ?? pd.description ?? '');
        setSystemPrompt(pd.systemPrompt ?? '');
        setModel(pd.model ?? '');
        setTemperature(typeof pd.temperature === 'number' ? String(pd.temperature) : '');
  ...
  return (
    <>
      ...
      <div className={styles.descfield}>
        <div className={styles.fl}>Label <em>· display name for this prompt</em></div>
        <input style={inputStyle} value={label} onChange={(e) => { setLabel(e.target.value); mark(); }} />
      </div>

      {/* Description */}
      <div className={styles.descfield}>
        <div className={styles.fl}>Description <em>· shown wherever this prompt is listed</em></div>
        <textarea
      ...
      <p style={{ color: 'var(--dim)', fontSize: 13, margin: '0 0 22px', maxWidth: '64ch' }}>
        A reusable writing-craft prompt you run against a book file from the Prompt Runner. The system prompt defines the task; the selected file's text is passed in as the user message.
      </p>
      ...
      {/* System prompt */}
      <div className={styles.descfield}>
        <div className={styles.fl}>System prompt <em>· defines the writing-craft task</em></div>
        <textarea
          value={systemPrompt}
