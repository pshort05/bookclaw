...
export async function runWordTargetContinuation(opts: {
  ...
  /** Produce the next continuation chunk for the given progress. */
  continue: (ctx: { wordsSoFar: number; remaining: number; pass: number }) => Promise<string>;
  ...
  while (wc < opts.wordCountTarget && passes < maxPasses) {
    ...
    let cont: string;
    try {
      cont = await opts.continue({ wordsSoFar: wc, remaining, pass: passes });
    } catch {
