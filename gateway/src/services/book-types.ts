...
export interface BookManifest {
  ...
  roleModels?: Record<string, { provider: string; model?: string }>; // "Apply to every <Role>" — per-STEP-ROLE model pins (keyed by StepRole, so a pin never bleeds onto a sibling role that happens to share a taskType). Resolved by stepRouting ABOVE a pipeline-baked template modelOverride and below an explicit per-step pin, so chapters expanded later inherit it (additive-optional, no schema bump)
  sceneBriefModel?: { provider: string; model?: string }; // Author-seeded default model for the scene_brief role; book-board editable; resolved by castStep above the genre casting sheet, below manual/write-screen pins (additive-optional, no schema bump)
  draftModel?: { provider: string; model?: string };      // Author-seeded default model for the draft role; book-board editable; resolved by castStep above the genre casting sheet, below manual/write-screen pins (additive-optional, no schema bump)
  alternateTakes?: { sceneTakes?: boolean; draftOpening?: boolean }; // Alternate Takes (Verbalized Sampling) per-book opt-in; injects a vs-enabled decision step before scene_brief / draft at expand time (additive-optional, no schema bump)
